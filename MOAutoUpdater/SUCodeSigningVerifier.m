...
+ (BOOL)codeSignatureIsValidAtPath:(NSString *)destinationPath error:(NSError **)error {
    ...
    if (result != 0) {
        MOLOG(@"Failed to copy host code %d", result);
        goto finally;
    }

    result = SecCodeCopyDesignatedRequirement(hostCode, kSecCSDefaultFlags, &requirement);
    if (result != 0) {
        MOLOG(@"Failed to copy designated requirement %d", result);
        goto finally;
    }

    newBundle = [NSBundle bundleWithPath: destinationPath];
    if (!newBundle) {
        MOLOG(@"Failed to load NSBundle for update");
        result = -1;
        goto finally;
    }
    ...
    if (result != 0) {
        MOLOG(@"Failed to get static code %d", result);
        goto finally;
    }
    ...
    if (result != 0 && error) {
        if (result == errSecCSReqFailed) {
            ...
            if (SecRequirementCopyString(requirement, kSecCSDefaultFlags, &requirementString) == noErr) {
                MOLOG(@"Failed requirement %@", requirementString);
